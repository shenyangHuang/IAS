...
def main():
    ...
    accuracy = main.train_Teacher(datasets[0], architecture=[16],input_dim=784, output_dim=2, lr=lr,epochs=epochs, batch_size=32, random_seed=seed,
                                    mute=True,test=True,class_curve=True)
    ...
    for i in range(2,10):
        ...
        SAS_accuracy = SAS_Controller.execute(datasets[0],input_dim=784,output_dim=(i+1),lr=lr,epochs=epochs,batch_size=32,random_seed=seed,mute=True,
        ...
        if(SAS_accuracy[1] < 0.90):
            base_accuracy = SAS_Controller.baseline(datasets[0],input_dim=784,output_dim=(i+1),lr=lr,epochs=epochs,batch_size=32,random_seed=seed,mute=True,
                                                            test=True,noVal=False,upload=True,class_curve=True)
            print ("replacing Net2Net weights with baseline weights due to complacency learning")
        ...
        SAS_best = MLP_SAS.simple_search(datasets[0],SAS_params,SAS_accuracy[1],N_workers,restart_num=restart_num,output_dim=(i+1),lr=lr,epoch_limit=epoch_limit,random_seed=seed)
